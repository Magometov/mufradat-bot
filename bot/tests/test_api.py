...
def test_path_becomes_a_public_address():
...
def test_card_without_a_picture_stays_without_one():
    """Пустой путь остаётся пустым: карточка уедет текстом."""
    assert api._image_url(None) is None


def test_without_the_public_address_the_picture_is_dropped(monkeypatch):
