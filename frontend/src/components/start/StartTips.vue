    ...
    <div :class="$style.StartTips" role="dialog" aria-modal="true" @click="handleNext">
        ...
        <div :class="$style.StartTips__box" @click.stop>
            ...
            <template v-else-if="step === 4">
                ...
                <p :class="$style.StartTips__text">
                    Слово идёт вперёд по этой лестнице, когда вспомнишь его {{ needed }}
                    {{ timeWord(needed) }} подряд — каждый раз другой стороной. Знакомые слова
            ...
            <template v-else>
                ...
                <p :class="$style.StartTips__text">
                    Слово, которое не далось, вернётся в этом же заходе, а счёт сторон начнётся
                    заново. Каждый промах отодвигает его чуть дальше, чтобы оно не мелькало перед
                    глазами.
                </p>
