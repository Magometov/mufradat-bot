...
# Опознавать неопознанного как одного человека — только для своей машины: в браузере
# подписи Telegram нет, а прогресс надо видеть. Признак отдельный, а не привязан к
# `SCHEDULING_FOR_ALL`: тот на сервере однажды станет `true`, и единственной защитой
# остался бы `DEBUG`. Здесь нужны оба, и второго в проде нет вовсе.
LOCAL_LEARNER = getenv("LOCAL_LEARNER", "").lower() == "true"
