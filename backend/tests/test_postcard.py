...
def test_letters_are_joined_and_reversed():
...
def test_long_words_are_carried_over():
...
def test_a_very_long_card_has_no_tail():
    ...
    card = Image.open(
        BytesIO(
            render(
                ...
                transliteration="bismi llāhi r-raḥmāni r-raḥīm wa-l-ḥamdu li-llāhi rabbi l-ʿālamīn",
            )
        )
    )
...
def test_scholarly_latin_has_glyphs(char):
...
def test_arabic_has_glyphs(char):
