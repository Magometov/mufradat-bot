...
def test_slot_starts_at_its_hour():
...
def test_before_the_first_slot_there_is_none():
    """До первого часа слота нет: ночью в группу не пишем."""
    assert group_slot(NIGHT) is None


def test_slots_are_read_from_the_constant(monkeypatch):
...
def test_one_word_per_slot(deck):
...
@GROUP
@pytest.mark.django_db
def test_deck_goes_around_without_repeats(deck):
...
@GROUP
@pytest.mark.django_db
def test_new_circle_starts_with_the_oldest(deck):
...
@GROUP
@pytest.mark.django_db
def test_a_new_word_jumps_the_queue(deck):
...
@GROUP
@pytest.mark.django_db
def test_phrases_stay_out(phrase):
...
@pytest.mark.django_db
def test_without_a_group_nothing_is_spent(deck):
...
@GROUP
@pytest.mark.django_db
def test_empty_deck_sends_nothing(db):
...
@GROUP
@pytest.mark.django_db
def test_deleted_word_takes_its_row(deck):
...
@GROUP
@pytest.mark.django_db
def test_the_sent_word_is_the_one_marked(deck):
...
@GROUP
@pytest.mark.django_db
def test_the_circle_moves_the_same_row(deck):
...
@GROUP
@pytest.mark.django_db
def test_a_late_ask_still_sends(deck):
...
@GROUP
@pytest.mark.django_db
def test_forced_ignores_the_schedule(deck):
...
@GROUP
@pytest.mark.django_db
def test_forced_moves_the_circle(deck):
