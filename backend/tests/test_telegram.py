...
class TestSignature:
    """Сверка подписи Telegram: чья она, свежая ли и есть ли в ней человек."""

    @settings
    def test_fresh_signature_gives_the_user(self):
        """Свежая подпись опознаёт человека: id и ник берутся из неё."""
        assert user_from(init_data()) == (42, "ali")

    @settings
    def test_signature_older_than_a_day_is_refused(self):
    ...
    @settings
    def test_signature_just_inside_the_window_still_works(self):
        """На границе срока подпись ещё действует."""
        assert user_from(init_data(age=SIGNATURE_MAX_AGE - timedelta(minutes=1))) is not None

    @settings
    def test_signature_from_the_future_is_refused(self):
    ...
    def test_without_token_nobody_is_recognised(self):
        """Без токена бота сверять нечем."""
        assert user_from(init_data()) is None

    def test_epoch_dates_are_handled(self):
