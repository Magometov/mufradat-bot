...
from apps.learning.rules import State
...
def test_first_sight_goes_far():
    ...
    state, due = rules.next_state(None, knows=True, now=NOW)
...
def test_unknown_new_card_goes_to_learning():
    ...
    state, due = rules.next_state(None, knows=False, now=NOW)
...
def test_first_correct_in_learning_only_counts():
    ...
    state, due = rules.next_state(State(level=0, step=0), knows=True, now=NOW)
...
def test_second_correct_closes_learning():
    ...
    state, due = rules.next_state(State(level=0, step=1), knows=True, now=NOW)
...
def test_miss_in_learning_resets_the_count():
    ...
    state, due = rules.next_state(State(level=0, step=1), knows=False, now=NOW)
...
def test_known_card_climbs_one_step():
    ...
    state, due = rules.next_state(State(level=3), knows=True, now=NOW)
...
def test_top_level_stays_on_top():
    ...
    state, due = rules.next_state(State(level=len(LADDER)), knows=True, now=NOW)
...
def test_forgotten_card_falls_to_learning():
    ...
    state, due = rules.next_state(State(level=5), knows=False, now=NOW)
...
def test_intervals_are_spread():
    ...
    spans = {
        days_between(rules.next_state(State(level=3), knows=True, now=NOW)[1]) for _ in range(20)
    }
...
def test_ladder_comes_from_settings():
    ...
    first, due = rules.next_state(None, knows=True, now=NOW)
    assert first == State(level=1)
    assert days_between(due) == 2

    second, due = rules.next_state(first, knows=True, now=NOW)
