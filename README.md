...
```
brew install libraqm     один раз: без него вязь на карточке-картинке рассыпается
docker compose up -d db  база для тестов
make test                тесты бэкенда, бота и фронтенда
```

Pillow ставится из исходников: в готовых колёсах нет raqm, а без него арабские буквы
рисуются порознь и огласовки съезжают с букв. Тесты `TestShaping` это и проверяют.
